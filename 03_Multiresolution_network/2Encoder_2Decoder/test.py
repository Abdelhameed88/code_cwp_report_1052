...
def evaluate(model, criterions, dataloader, device, k, ctx, vis_path, vis_batch, vis_sample, missing, std) -> None:
    ...
    label1_pred_list, label2_pred_list = [], []
        
    label1_list, label2_list = [], []

    # store normalized prediction & gt in tensor
    label1_tensor, label2_tensor  = [], []
    
    label1_pred_tensor, label2_pred_tensor  = [], []
    ...
    with torch.no_grad():
        batch_idx = 0
        for data_y, data_x, label1, label2 in dataloader:
            ...
            if missing or std:
                ...
                pred_lb1, pred_lb2 = model(data_y_noise, data_x_noise)
            else:
                pred_lb1, pred_lb2 = model(data_y, data_x)
