...
# torch.manual_seed(333310)
# np.random.seed(333310)
# random.seed(333310)
...
def update_csv(filename, epoch, loss, loss_g1_lb1, loss_g1_lb2,  \
                                         loss_g2_lb1, loss_g2_lb2, \
                             train_loss, train_loss_g1_lb1, train_loss_g1_lb2,  \
                                         train_loss_g2_lb1, train_loss_g2_lb2):
    file_exists = os.path.isfile(filename)
    with open(filename, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        if not file_exists:
            writer.writerow(["Epoch", "val_loss", "val_loss_g1_lb1", "val_loss_g1_lb2", \
                                                  "val_loss_g2_lb1", "val_loss_g2_lb2", \
                             "tr_loss", "tr_loss_g1_lb1", "tr_loss_g1_lb2", \
                                        "tr_loss_g2_lb1", "tr_loss_g2_lb2"])
        writer.writerow([epoch, loss, loss_g1_lb1, loss_g1_lb2, \
                                      loss_g2_lb1, loss_g2_lb2, \
                         train_loss, train_loss_g1_lb1, train_loss_g1_lb2, \
                                     train_loss_g2_lb1, train_loss_g2_lb2])
...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, dataloader, device, epoch, print_freq, writer, args):
    ...
    for data_Vcomp, data_Hcomp, label1, label2 in metric_logger.log_every(dataloader, print_freq, header):
        ...
        output_lb1, output_lb2  = model(data_Vcomp, data_Hcomp)

        loss, loss_g1_lb1, loss_g1_lb2, \
              loss_g2_lb1, loss_g2_lb2  = criterion(output_lb1, output_lb2, \
                                                                            label1, label2)
        ...
        loss_val = loss.item()
        loss_g1_val_lb1, loss_g1_val_lb2  = loss_g1_lb1.item(), loss_g1_lb2.item()
        loss_g2_val_lb1, loss_g2_val_lb2  = loss_g2_lb1.item(), loss_g2_lb2.item()
        batch_size = data_Vcomp.shape[0]
        
        metric_logger.update(loss=loss_val, loss_g1_lb1 = loss_g1_val_lb1, loss_g1_lb2 = loss_g1_val_lb2, \
                             loss_g2_lb1 = loss_g2_val_lb1, loss_g2_lb2 = loss_g2_val_lb2, \
    ...
    return metric_logger.loss.global_avg, metric_logger.loss_g1_lb1.global_avg, metric_logger.loss_g1_lb2.global_avg, \
                                          metric_logger.loss_g2_lb1.global_avg, metric_logger.loss_g2_lb2.global_avg
...
def evaluate(model, criterion, dataloader, device, writer, args):
    ...
    with torch.no_grad():
        for data_Vcomp, data_Hcomp, label1, label2 in metric_logger.log_every(dataloader, 20, header):
            ...
            metric_logger.update(loss=loss.item(), loss_g1_lb1=loss_g1_lb1.item(), loss_g1_lb2=loss_g1_lb2.item(), \
                                                   loss_g2_lb1=loss_g2_lb1.item(), loss_g2_lb2=loss_g2_lb2.item())
...
def main(args):
    ...
    def freeze_encoders_decoders(model):
        ...
        for name, param in model.named_parameters():
            ...
            if any(layer in name for layer in ['lstm', 'convblock8']):
            # if any(layer in name for layer in ['lstm', 'convblock8', 'deconv3_1', 'deconv3_2', 'deconv1_1', 'deconv1_2', 'deconv6']):
    ...
    def criterion(pred_lb1, pred_lb2, gt_lb1, gt_lb2):
        ...
        loss = (args.lambda_g1_lb1 * loss_g1_lb1) + (args.lambda_g1_lb2 * loss_g1_lb2) + \
               (args.lambda_g2_lb1 * loss_g2_lb1) + (args.lambda_g2_lb2 * loss_g2_lb2)
                
        return loss, loss_g1_lb1, loss_g1_lb2, \
                     loss_g2_lb1, loss_g2_lb2
    ...
    if args.resume:
        ...
        # Modify the state_dict to remove unnecessary decoders
        state_dict_6Decoders = checkpoint['model']
        state_dict_2Decoder = model_without_ddp.state_dict()  # Get the current model's state_dict
        # Remove any keys from the state_dict that do not match the model
        keys_to_remove = [key for key in state_dict_6Decoders if key not in state_dict_2Decoder]
        for key in keys_to_remove:
            del state_dict_6Decoders[key]
        # Now load the modified state_dict
        model_without_ddp.load_state_dict(state_dict_6Decoders, strict=False)

        # Step 2: Reinitialize the optimizer
        lr = args.lr * args.world_size
        optimizer = torch.optim.AdamW(model_without_ddp.parameters(), lr=lr, betas=(0.9, 0.999), weight_decay=args.weight_decay)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if int(os.environ.get('SLURM_PROCID', 0)) == 0:
            update_csv("loss_func.csv", epoch, loss, loss_g1_lb1, loss_g1_lb2, \
                                                     loss_g2_lb1, loss_g2_lb2, \
                        train_loss, train_loss_g1_lb1, train_loss_g1_lb2,\
                                    train_loss_g2_lb1, train_loss_g2_lb2)
