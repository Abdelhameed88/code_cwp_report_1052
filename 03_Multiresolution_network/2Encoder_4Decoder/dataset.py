...
class FWIDataset(Dataset):
    def __init__(self, anno, preload=True, sample_ratio=1, file_size=1,
                 ...
                 transform_label1=None, transform_label2=None, transform_label3=None, transform_label4=None):
        ...
        self.transform_label3 = transform_label3
        self.transform_label4 = transform_label4
        with open(anno, 'r') as f:
            self.batches = f.readlines()
        if preload:
            self.data_y_list, self.data_x_list, self.label1_list,  self.label2_list,  self.label3_list,  self.label4_list = [], [], [], [], [], []
            for batch in self.batches:
                data_y, data_x, label1, label2, label3, label4 = self.load_every(batch)  
                ...
                if label3 is not None:
                    self.label3_list.append(label3)
                if label4 is not None:
                    self.label4_list.append(label4)
            ...
            print(f"Label3 max: {np.max(label3)}", f"Label3 min: {np.min(label3)}")
            print(f"Label4 max: {np.max(label4)}", f"Label4 min: {np.min(label4)}")
    ...
    def load_every(self, batch):
        ...
        if len(batch) > 2:
            ...
            label3_path = batch[4][:-1]
            label3 = np.load(label3_path)
            label3 = label3.astype('float32')

            label4_path = batch[5][:-1]
            label4 = np.load(label4_path)
            label4 = label4.astype('float32')

        else:
            label1, label2, label3, label4 = None, None, None, None

        return data_y, data_x, label1, label2, label3, label4
    ...
    def __getitem__(self, idx):
        ...
        if self.preload:
            ...
            if self.file_size == 1:
                ...
                label3 = self.label3_list[batch_idx] if len(self.label3_list) != 0 else None
                label4 = self.label4_list[batch_idx] if len(self.label4_list) != 0 else None
                ...
                if label2 is not None and label2.ndim == 4:
                    label2 = label2[None, ...]
                if label3 is not None and label3.ndim == 4:
                    label3 = label3[None, ...]
                if label4 is not None and label4.ndim == 4:
                    label4 = label4[None, ...]
            else:
                ...
                if len(self.label2_list) != 0:
                    ...
                    label2 = None
                if len(self.label3_list) != 0:
                    label3_arr = self.label3_list[batch_idx]
                    if label3_arr.ndim == 5:
                        label3_arr = label3_arr.squeeze(0)
                    label3 = label3_arr[sample_idx]
                    label3 = label3[None, ...]
                else:
                    label3 = None
                if len(self.label4_list) != 0:
                    label4_arr = self.label4_list[batch_idx]
                    if label4_arr.ndim == 5:
                        label4_arr = label4_arr.squeeze(0)
                    label4 = label4_arr[sample_idx]
                    label4 = label4[None, ...]
        else:
            ...
            if self.file_size == 1:
                ...
                label3 = label3 if label is not None else None
                label4 = label4 if label is not None else None
                if label1 is not None and label1.ndim == 4:
                    label1 = label1[None, ...]
                if label2 is not None and label2.ndim == 4:
                    label2 = label2[None, ...]
                ...
                label3 = label[0][sample_idx] if label is not None else None
                label4 = label[0][sample_idx] if label is not None else None
                if label1 is not None and label1.ndim == 3:
                    label1 = label1[None, ...]
                if label2 is not None and label2.ndim == 3:
                    label2 = label2[None, ...]
                if label3 is not None and label3.ndim == 3:
                    label3 = label3[None, ...]
                if label4 is not None and label4.ndim == 3:
                    label4 = label4[None, ...]
        ...
        if self.transform_label3:
            label3 = self.transform_label3(label3)
        if self.transform_label4:
            label4 = self.transform_label4(label4)
...
if __name__ == '__main__':
    ...
    transform_label3 = Compose([
        t.MinMaxNormalize(1950, 3930)
    ])
    transform_label4 = Compose([
        t.MinMaxNormalize(1950, 3930)
    ])
    ...
    dataset = FWIDataset('relevant_files/temp.txt',
                         ...
                         transform_label3=transform_label3, 
                         transform_label4=transform_label4, 
                         file_size=1)

    data_y, data_x, label1, label2, label3, label4 = dataset[0]
    ...
    print("label3 shape:", label3.shape if label3 is not None else None)
    print("label4 shape:", label4.shape if label4 is not None else None)